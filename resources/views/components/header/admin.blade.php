<header class="bg-black">
    <div class="flex justify-between text-white">
        ...
        <div class="flex gap-x-3 flex-1 justify-between">
            <a href="{{route('admin.dashboard.view')}}" class="text-2xl btn btn-dark flex items-center">{{ __('admin.content_library')}}</a>
            <div class="h-full flex">
                ...
                @endif
                @if(Auth::user())
                    <a class="btn btn-dark flex items-center" href="{{ route('admin.user.view', strtolower(Auth::user()->username)) }}" >{{ Auth::user()->name; }}</a>
                @endif
            </div>
        </div>
    </div>
